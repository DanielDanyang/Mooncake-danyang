...
class MooncakeConnectorScheduler:
    ...
    def request_finished(
        ...
        if delay_free_blocks:
            _mc_timeline(
                "pd_request_finished_delay_free",
                req_id=request.request_id,
                transfer_id=params.get("transfer_id"),
                num_blocks=len(block_ids),
            )
            pd_control.mark_pending(1, params.get("transfer_id"))
            _mc_timeline(
                "pd_control_pending",
                req_id=request.request_id,
                transfer_id=params.get("transfer_id"),
...
class MooncakeConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        logger.info(
            "The Mooncake Transfer Engine device_name is %r.", device_name
        )
    ...
    async def send_kv_to_decode(
        ...
        while wait_tasks:
            ...
            _mc_timeline(
                "pd_build_params_begin",
                ready_reqs=len(ready_reqs),
            ...
            if src_ptrs:
                remote_session = f"{meta.remote_hostname}:{meta.remote_port}"
                total_bytes = sum(lengths)
                pd_control.mark_pending(total_bytes)
                _mc_timeline(
                    "pd_write_submit",
    ...
    def _send_blocks(
        ...
        pd_control.begin_pd(total_bytes)
        _mc_timeline(
            "pd_write_begin",
            remote_session=remote_session,
            descs=len(lengths),
