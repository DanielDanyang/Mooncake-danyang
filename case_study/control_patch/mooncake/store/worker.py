...
from vllm.distributed.kv_transfer.kv_connector.v1.mooncake.store.pd_control import (
    pd_control,
)
...
def _mc_timeline(event: str, **kw: Any) -> None:
    path = os.getenv("MOONCAKE_TIMELINE_PATH")
    if not path:
        return
    rec = {
...
class KVTransferThread(threading.Thread):
    ...
    def add_request(self, request: ReqMeta) -> None:
        _mc_timeline(
            ...
            token_len=getattr(request, "token_len_chunk", None),
            can_save=getattr(request, "can_save", None),
        )
...
class KVCacheStoreSendingThread(KVTransferThread):
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        if current_event is not None:
            _mc_timeline("store_cuda_event_sync_begin", req_id=req_id)
            current_event.synchronize()
            _mc_timeline("store_cuda_event_sync_end", req_id=req_id)

        total_bytes = sum(sum(s) if isinstance(s, list) else s for s in sizes)
        if pd_control.mode == "delay_store":
            snap_before = pd_control.snapshot()
            _mc_timeline(
                "store_control_wait_begin",
                req_id=req_id,
                mode=pd_control.mode,
                pending=snap_before["pending"],
                active=snap_before["active"],
                total_bytes=total_bytes,
            )
            wait_res = pd_control.wait_for_pd_quiet()
            snap_after = pd_control.snapshot()
            _mc_timeline(
                "store_control_wait_end",
                req_id=req_id,
                mode=pd_control.mode,
                wait_ms=wait_res.waited_s * 1000.0,
                reason=wait_res.reason,
                pending=snap_after["pending"],
                active=snap_after["active"],
                total_bytes=total_bytes,
            )
        _mc_timeline(
            "store_put_begin",
        ...
        try:
            ...
            _mc_timeline(
                ...
                nkeys=len(keys),
                total_bytes=total_bytes,
                failed=sum(1 for v in res if v < 0),
                min_ret=min(res) if res else None,
            )
        ...
        if self.kv_role in ["kv_producer", "kv_both"]:
            ...
            for request in meta.requests:
                ...
                _mc_timeline(
                    ...
                    token_len=request.token_len_chunk,
                    num_blocks=len(request.block_ids or ()),
                )
